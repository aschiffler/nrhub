...
class AzureAdOAuthenticator(OAuthenticator):
    ...
    admin_group = Unicode(config=True)
    @default('tenant_id')
    ...
    async def authenticate(self, handler, data=None):
        ...
        userdict = {"name": decoded[self.username_claim].split('@')[0]+'-'+decoded[self.username_claim].split('@')[1]}
        userdict["auth_state"] = auth_state = {}
        userdict['admin'] = (self.admin_group in (decoded['groups'] if decoded.get('groups') else [] ))
